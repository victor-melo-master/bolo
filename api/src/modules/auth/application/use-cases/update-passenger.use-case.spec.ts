...
describe('UpdatePassengerUseCase', () => {
  ...
  const mockPassenger = new Passenger(
    'passenger-id', '+584141234500', 'old@email.com', 'hash',
    'Pasajero Viejo', 'V12345678', null, 'normal', false, true,
  ...
  it('should update passenger fields and return updated profile', async () => {
    ...
    const updatedPassenger = new Passenger(
      'passenger-id', '+584141234500', 'new@email.com', 'hash',
      'Pasajero Nuevo', 'V87654321', null, 'student', false, true,
      null, null, new Date(), new Date(),
    );
    passengerRepo.save.mockResolvedValue(updatedPassenger);

    const dto = { fullName: 'Pasajero Nuevo', email: 'new@email.com', cedula: 'V87654321', category: 'student' };
    const result = await useCase.execute('passenger-id', dto);

    expect(passengerRepo.findById).toHaveBeenCalledWith('passenger-id');
    expect(passengerRepo.save).toHaveBeenCalledWith(expect.objectContaining(dto));
